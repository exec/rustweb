...
impl PrometheusExporter {
    ...
    pub fn export_metrics(&self) -> Response<Full<Bytes>> {
        ...
        output.push_str("# HELP rustweb_request_duration_milliseconds Total time spent processing requests\n");
        output.push_str("# TYPE rustweb_request_duration_milliseconds counter\n");
        output.push_str(&format!("rustweb_request_duration_milliseconds {}\n", snapshot.requests_duration_ms));
        output.push('\n');
        ...
        // Status code metrics
        output.push_str("# HELP rustweb_requests_by_status_total Total requests by HTTP status code\n");
